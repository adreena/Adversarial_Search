...
def _play(agents, game_state, time_limit, match_id):
    ...
    while not game_state.terminal_test():
        ...
        try:
            action = fork_get_action(game_state,match_id, players[active_idx], time_limit)
        except Empty:
            logger.info(
    ...
    return winner, game_history, match_id, agents, initial_state


def fork_get_action(game_state, match_id, active_player, time_limit):
    action_queue = Queue()
    ...
    p = Process(target=_request_action, args=(active_player, match_id, game_state, time_limit, client))
    p.start()
...
def _request_action(agent, match_id, game_state, time_limit, conn):
    ...
    try:
        agent.get_action(game_state, match_id)
    except StopSearch:
        pass
