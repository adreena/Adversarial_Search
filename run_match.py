...
def _run_matches(matches, name, num_processes=NUM_PROCS):
    ...
    for result in pool.imap_unordered(play, matches):
        winner, game_history, match_id, agents, initial_state = result
        print("+" if winner.name == name else '-', end="")
        # first_agent = agents[0]
        # second_agent =  agents[1]
        # state_str = "{0:b}".format(initial_state.board)
       
        # my_player_id = 0
        # hash_key = (state_str, None, None, my_player_id)
        # my_first_action = game_history[0]

        # if second_agent.name == name:
        #     my_player_id = 1
        #     hash_key = (state_str, game_history[0], None, my_player_id)
        #     my_first_action = game_history[1]
        
        # reward = -1
        # if winner.name == name:
        #     reward = 1
        # # print('\n round:', hash_key in book.keys(), hash_key)   
        # book[hash_key][my_first_action] += reward  

        results.append(result)

    # book_result = {k: max(v, key=v.get) for k, v in book.items()}
    # with open("data.pickle", 'wb') as f:
    #     pickle.dump(book_result, f)
...
def make_fair_matches(matches, results):
    new_matches = []
    for _, game_history, match_id, _, _ in results:
